...
"Teachers reported on students' classroom behaviors relevant to academic success using the Teacher-Child Rating Scale (TCRS; Hightower, 1986). Each item on the task orientation (e.g., “well-organized”, “com- pletes work”, “works well without adult support”) and frustration tolerance (e.g., “accepts things not going his/her way”, “copes well with failure”) scales was rated on a five-point scale, ranging from 1 = “not at all” to 5 = “very well”."
...
Because this is a reproduction using the data from the original study, I do not anticipate any differences in sample, setting, procedure, or analysis plan. The only differences I foresee involve the actual R code used in the analyses, as I will be developing my code from scratch.
...
Without having seen the data, it is not currently possible to outline all of the steps that will be needed to transform it into a tiny format, nor all of the new columns that will need to created, altered, and/or removed. Nevertheless, I already anticipate a need to create columns for the following variables: EF test accuracy composite score, EF test RT composite score, and teacher-reported self-regulated classroom behaviors composite score.
